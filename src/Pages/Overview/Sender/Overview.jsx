...
export default function Overview({url}){
    ...
    useEffect(()=>{
        socket.emit("join_room", messageId)
        axios.get(`${url}/api/infomsgovw/${messageId}`).then(res=>{
            setText(res.data.text)
            setDate(tanggal(res.data.time))
        }).catch(err=>alert(err))
        axios.get(`${url}/api/reply/${messageId}`).then(res=>{
            if(res.data === 0){
                return setData([])
            }
            setData(res.data)
    ...
    useEffect(()=>{
        socket.on("private_message", dataS =>{
            setData([...data, dataS])
            scroll.current.scrollIntoView()
        })
    },[socket, data])
                ...
                <div className="col-12 col-md-5 d-flex flex-column" style={{maxHeight: '80vh', overflow:"auto", borderRadius: '10px'}}>
                    ...
                    {data ? data.map((obj, i)=>{
                        if(obj.sender === "stranger"){
                            return (<SenderReply text={obj.text} key={i} date={tanggal(obj.date)} />)
                        }else{
                            return (<ReceiveReply text={obj.text} key={i} date={tanggal(obj.date)} send={obj.sender}/>)
                        }
                    }): "no Message Yet"}
                    <div ref={scroll}></div>
                    <div>
